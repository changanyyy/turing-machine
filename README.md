...
众所周知，冯诺依曼计算机是图灵机的改进实现，程序就是图灵机中的纸带。为了模拟计算机运⾏，需要⼤家使⽤ 附录中描述的图灵机程序语法，实现图灵机程序以计算最⼤公约数。 

### 说明：

1. 实现函数功能：求两个整数的最⼤公约数。 
2. 输⼊两个整数，我们⽤1的数量来表示输⼊的正整数，⽤0来分隔两个输⼊整数。例如整数4表示为1111，整数5表示11111。因此若需要求4和5的公约数，其输⼊的串为1111011111。类似的，输出的公约数同样⽤1的个数来表示。 

### 要求： 

1. 将图灵机输⼊的串经过计算得到最⼤公约数。 
2. 实现的图灵机程序中的纸带数 N > 1 。 
3. 使⽤你在任务⼀中实现的图灵机程序解析器对你实现的图灵机程序进⾏解析，得到对应的模拟器。 
4. 使⽤模拟器对合法的输⼊串进⾏模拟运⾏。对于运⾏得到的最⼤公约数，在第⼀条纸带上进⾏打印；要求在图灵机停⽌运⾏时，第⼀条纸带上不出现最⼤公约数外的其他内容。 
5. 请将你的图灵机设计思路以及每个状态表达的意义以注释的形式在图灵机程序⽂件中进⾏说明。
## 附录 图灵机程序语法
...
完整的图灵机程序示例参见 *[palindrome_detector_2tapes.tm](https://ptc-nju.github.io/examples/palindrome_detector_2tapes.tm)*。